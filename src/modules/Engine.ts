...
export default class Engine {
      ...
      if (this._activeMap.isImageLoaded) {
        ...
        if (elapsed >= fpsInterval) {
          ...
          Object.values(this._activeMap.gameObjects)
            .sort((gameObjectA, gameObjectB) => {
              return gameObjectA.y - gameObjectB.y;
            })
            .forEach((gameObject) => {
              !gameObject.isHidden &&
                gameObject.objectSprite.draw(
                  this._ctx,
                  this._cameraView,
                  this._activeMap.gameObjects.miniMe
                );
              gameObject.door &&
                gameObject.door.draw(
                  this._ctx,
                  this._cameraView,
                  this._activeMap.gameObjects.miniMe
                );
  ...
  public bindCheckCharacterPosition() {
    document.addEventListener("CharacterWalkComplete", (event: CustomEvent) => {
      ...
      if (
        !this._activeMap.isInteracting &&
        this._activeMap.checkForInteraction()
      ) {
        this._showHelper();
      } else {
        this._hideHelper();
    ...
    document.addEventListener("CharacterDrunk", () => {
      this._activeMap.startInteraction([
        {
          type: "message",
          text: `But something feels... off. It seems like you need a rest`,
        },
      ]);
    ...
    document.addEventListener("CharacterSober", () => {
      this._activeMap.startInteraction([
        {
          type: "message",
          text: `You take a short rest, and the world finally stops spinning – back to normal!`,
        },
      ]);
      this._directionInput.keyMap = {
        ArrowUp: "up",
  ...
  public startMap(mapName: string) {
    ...
    const cameraViewObject =
      Object.values(this._activeMap.gameObjects).find(
        (gameObject) => gameObject.isCameraView
      ) || this._activeMap.gameObjects.miniMe;
  ...
  public init() {
    ...
    document.addEventListener("mapImageLoaded", () => {
      const numberMapsLoaded = Object.values(this._maps).filter(
        (map) => map.isImageLoaded
      ).length;
  ...
  public resizeCanvas() {
    ...
    const scale = window.location.search.includes("debug")
      ? 0.9
      : CANVAS_SCALE[this._gameContainer.clientWidth];
    ...
    this._helperText.textContent = `Press ${
      window.isMobile ? "A" : "spacebar"
    } to interact`;
