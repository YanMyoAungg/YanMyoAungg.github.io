...
export class ChatApi {
  ...
  private buildSystemMessage(
    ...
    const content = [profileCustom, "", "---", "FACTS ABOUT ME:", facts].join(
      "\n",
    );
  ...
  async sendMessage(userMessage: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error(
          "Invalid API key. Check your OpenRouter key in src/configs/apiKey.ts.",
        );
      }
      if (response.status === 404) {
        throw new Error(
          "AI model not found. The free model may have changed. Try updating DEFAULT_MODEL in src/utils/chatApi.ts.",
        );
      }
      throw new Error("Connection lost. Please try again.");
    }
